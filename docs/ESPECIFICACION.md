...
- **Etapa B (siguiente PR): el planificador.** Cada minuto pg_cron calcula bajo `postgres` —el mismo nivel de confianza que las migraciones; ninguna consulta de la app se salta la RLS— los recordatorios que tocan (huso del perfil, día programado, HH:MM exacto), se apunta en una tabla de deduplicación y entrega los envíos a `pg_net`, que hace POST a `/api/push/send` con un secreto compartido (`PUSH_DISPATCH_SECRET`). **El endpoint no toca la base de datos**: recibe los payloads resueltos, hace la criptografía Web Push (librería `web-push`) y responde estados; las suscripciones muertas (410) se limpian desde SQL con la respuesta que pg_net deja registrada.
