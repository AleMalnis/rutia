...
- **Tabla `push_subscriptions` (migración 0007)** con la RLS de siempre: endpoint único global, claves `p256dh`/`auth` del navegador, cascada con el usuario. La suscripción funciona como un secreto (quien la tenga puede enviarte avisos): entra por server action validada con Zod y no vuelve entera al cliente. Si el endpoint ya es de otro usuario (dispositivo compartido, p. ej. la demo), la RLS impide pisarlo: el cliente se desuscribe en local, obtiene un endpoint nuevo y reintenta una vez; la fila huérfana la limpiará el barrido de muertos de la etapa B.
