...
- Modo MCP avanzado: refresco del calendario **en tiempo real** (Supabase Realtime) mientras chateas desde tu cliente externo. (La pantalla de revocación que acompañaba a esta línea ya está hecha: §12.9.)
...
- ~~PWA instalable~~ → **implementada** (ver §4, «Instalación en el móvil»). Promovida desde este Could al decidirse que la vía móvil de la v1 es la PWA: de las rutas evaluadas (PWA, envoltorio WebView, app nativa, tiendas), es la única que entrega valor en días y sin coste fijo; el análisis completo y las descartadas quedan en §12. El **push** está en marcha como etapa 2 con sus tres decisiones tomadas (ver §4 «Avisos push»): la suscripción por dispositivo ya existe y falta el planificador (etapa B), que es cuando los términos se reescriben.
...
- **Afinado visual (tanda 4): acabado de coherencia** — los restos registrados al cerrar las tandas 1-3:
  - *Foco visible unificado*: un `:focus-visible` global en `globals.css` pinta el foco de teclado con el **acento del tema** (2 px, offset 2) en toda la app, login y legales incluidos — antes solo dos diálogos lo tenían y el resto dependía del anillo por defecto, que cada navegador pinta de un color ajeno a los temas. `:focus-visible` y no `:focus` a propósito (el clic no lo dispara); los inputs de texto conservan además su `focus:` por utilidad, y los que no lo tenían (formulario de ítem, categorías) lo ganan. La regla vive en `@layer base`: es un default que cualquier utilidad anula — sin capa pisaría el `outline-none` deliberado de los contenedores de diálogo y el teclado vería un anillo alrededor del diálogo entero.
  - *Un solo tamaño de input*: todos los campos de texto pasan a `px-2.5 py-1.5` (formulario de ítem y categorías iban a `px-2` mientras IA y borrado usaban `px-2.5`). Los botones compactos de fila (`text-xs`, `px-2`) son una talla propia y deliberada, no una inconsistencia.
  - *Se eliminan los SVG huérfanos de la plantilla* (`file.svg`, `globe.svg`, `window.svg` en `public/`): cero referencias desde que la portada dejó de ser la de `create-next-app` (§12.16).
...
**Avisos push (etapa 2 de la PWA — decisiones tomadas en agosto de 2026):** recordatorios que suenan a su hora aunque la app esté cerrada. Tres decisiones de producto fijadas antes de escribir código: los términos se reescriben (prometían «no hay avisos»; ahora los hay, **opt-in** y sin garantía de entrega), el planificador es **pg_cron dentro de Supabase** (Vercel Hobby dispara con hora imprecisa y GitHub Actions añade infra ajena con retrasos variables; riesgo aceptado del plan Free: si nadie usa la app ~7 días el proyecto se pausa y el cron con él), y la pantalla de bloqueo enseña **solo el título y la hora, nunca el detalle** — la política ya avisa de que el detalle puede llevar datos de salud, y la pantalla de bloqueo la mira cualquiera. Diseño por piezas:
- **Solo recordatorios (chips)**: un bloque es ambiente; un recordatorio es una alerta por naturaleza (Must #9, medicación). El aviso sale a la hora del ítem en el huso del perfil.
- **Opt-in por dispositivo, campanita en el panel «Hoy»** (contextual: los avisos son sobre lo de hoy). Cada navegador es una suscripción; activar pide el permiso y la guarda, desactivar la borra, y «Probar» envía un aviso real al propio dispositivo — la única forma honesta de saber que funciona antes de necesitarlo. En iOS solo existe con la app instalada (≥16.4): fuera de eso la campanita no se ofrece.
- **Tabla `push_subscriptions` (migración 0007)** con la RLS de siempre: endpoint único global, claves `p256dh`/`auth` del navegador, cascada con el usuario. La suscripción funciona como un secreto (quien la tenga puede enviarte avisos): entra por server action validada con Zod y no vuelve entera al cliente. Si el endpoint ya es de otro usuario (dispositivo compartido, p. ej. la demo), el índice único lo rechaza (23505) y la action lo distingue de un re-alta propio: el cliente se desuscribe en local, obtiene un endpoint nuevo y reintenta una vez; la fila huérfana la limpiará el barrido de muertos de la etapa B.
- **VAPID en env del servidor** (`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`); la pública viaja como prop desde el servidor — mismo patrón que `MCP_RESOURCE_URL`, nada `NEXT_PUBLIC_`. Sin claves configuradas la campanita no se pinta: no se ofrece lo que el despliegue no tiene.
- **SW mínimo en `public/sw.js`**: maneja `push` (muestra título + hora, `tag` por ítem y día para no apilar duplicados) y `notificationclick` (enfoca o abre `/app`). **Sin caché ni offline**: siguen descartados como en la etapa 1. El *matcher* del proxy lo excluye, como al manifest. El contenido del push viaja cifrado extremo a extremo (`aes128gcm`): el servicio de push del navegador (Google/Mozilla/Apple) ve que llega un aviso, no lo que dice.
- **Etapa B, el planificador (migración 0008).** Cada minuto pg_cron calcula bajo `postgres` —el mismo nivel de confianza que las migraciones; ninguna consulta de la app se salta la RLS— los recordatorios que tocan: huso del perfil, día programado, y el minuto actual **más el anterior** (un tick que llega tarde no pierde el aviso; `push_sent`, con RLS sin políticas, deduplica y guarda una semana). Los envíos van vía `pg_net` a `/api/push/send` en trozos de como mucho 500 (un minuto gigante no tumba el lote entero), con la URL y el secreto en **Vault** (`push_dispatch_url` / `push_dispatch_secret` — el repo es público y una migración no puede llevar secretos). La entrega es **a-lo-sumo-una-vez** a propósito: si un envío falla, ese aviso se pierde — con TTL de 5 minutos reintentarlo tarde molesta más que ayuda, y el panel «Hoy» sigue siendo la fuente de verdad. **El endpoint no toca la base de datos**: valida el secreto en tiempo constante, hace la criptografía Web Push (`web-push`, TTL de 5 minutos: un recordatorio tardío ya no recuerda nada) y responde qué suscripciones dieron 404/410; el tick siguiente las barre con la respuesta que pg_net deja en `net._http_response`. Sin secretos en Vault, el trabajo avisa y no envía: no rompe nada.
...
- **Revocación.** En la app: la pestaña «Conectores» del diálogo de IA lista los accesos concedidos y cada uno se revoca ahí mismo (`listGrants` / `revokeGrant` del SDK, resuelto en §12.9); también sigue siendo posible desde el propio cliente.
...
- **El desfase de reloj de Supabase se absorbe en el borde** (visto en producción, agosto 2026): GoTrue y PostgREST corren en máquinas distintas y el reloj de PostgREST puede ir unos segundos por detrás, así que un token recién emitido —justo tras el login o un refresco de sesión— se rechaza con `PGRST303` «JWT issued at future» y la primera carga de `/app` moría en la pantalla de reintentar. No podemos sincronizar sus relojes; sí esperar a que el `iat` deje de ser futuro: el `fetch` de los clientes de servidor (`lib/supabase/server.ts` y `mcp.ts`) reintenta **solo** esa respuesta con pausas de 1 s y 2 s. Es seguro para cualquier método, también escrituras, porque PGRST303 se rechaza antes de ejecutar nada; cualquier otro error (un `PGRST301` caducado, un fallo de RLS) pasa tal cual y sin espera. Como se midieron ráfagas de desfase que agotaban los 3 s de reintentos, la **migración 0009** añade el arreglo de raíz: el hook de emisión retrasa el `iat` **60 s en todos los tokens** — el desfase deja de existir salvo que supere el minuto. Es seguro porque GoTrue solo valida el esquema de las claims tras el hook (tipos y presencia, verificado en su código fuente) y firma lo que el hook devuelva; `exp` no se toca (misma vida de token) y ningún consumidor nuestro valida `iat` contra el reloj. El reintento del fetch se queda como segunda red para ráfagas mayores.
...
9. ~~**Pantalla de revocación del modo MCP, pendiente**~~ — **resuelta** (se conserva el número): la pestaña «Conectores» del diálogo de IA lista los **accesos concedidos** (`auth.oauth.listGrants()`) y cada uno se revoca ahí mismo (`revokeGrant()`), sin migración ni cambios de dominio, tal como estaba previsto. Decisiones: carga perezosa al entrar en la pestaña (la mayoría abre el diálogo solo para la clave, y consultar grants es un viaje a GoTrue); confirmación en dos pasos como «Borrar clave», desarmada al cambiar de pestaña; tras revocar, la lista se repinta con la verdad del servidor, no con una resta local; y la misma **ventana honesta** que el borrado de cuenta — revocar mata sesiones y refresh tokens del cliente, pero un token de acceso ya emitido sigue valiendo hasta caducar (≤1 h) y aquí los datos siguen existiendo, así que se dice en la pantalla y en la política en vez de esconderse.
...
12. ~~**Consentimiento explícito del artículo 9, sin recoger aparte**~~ — **resuelta** (se conserva el número): el consentimiento del art. 9.2.a es ahora explícito, específico y demostrable. Decisiones:
    - **La regla vive en el servicio, no en la UI**: `createItem`, `updateItem` y `bulkCreateItems` rechazan (razón `consent`) cualquier guardado cuyo resultado lleve texto en *detalle* o *notas* si el usuario no tiene consentimiento registrado — así las **tres puertas** (formulario, chat, MCP) quedan cubiertas, no solo la que tiene casilla. En chat y MCP el mensaje de la herramienta le dice al usuario dónde marcarla; un ítem sin texto libre nunca la pide.
    - **Casilla en el formulario de ítem**, visible solo cuando hace falta (hay texto en detalle/notas y no hay consentimiento previo). Marcarla registra **una fila auditable** en `health_consents` (migración 0010): usuario, versión del texto aceptado (`HEALTH_CONSENT_VERSION`, en `lib/legal.ts` junto a la fecha de la política) y marca de tiempo. RLS de solo inserción y lectura propias: **ni update ni delete** — un consentimiento no se edita; solo cae con la cuenta (cascada). `profiles.preferences` se descartó a propósito: un consentimiento tiene que quedar auditable.
    - **Retirada, con precisión**: borrar o editar el texto elimina el dato y cesa su tratamiento; el consentimiento registrado **sigue en pie** para aportaciones futuras (cada una es voluntaria — el modelo es consentir la categoría de tratamiento una vez, no cada escritura). La retirada total —que la app vuelva a pedirlo— se atiende por correo eliminando el registro, como el resto de derechos atendidos. Se descartó a propósito un modelo de eventos aceptar/retirar con estado activo: maquinaria que ningún flujo real necesita hoy.
    - **`accepted_at` lo pone la base de datos** (trigger `before insert`): un default se puede pisar por PostgREST y un registro auditable no puede llevar fecha elegida por el interesado. La versión sí viene de la app: una inventada es inerte (el servicio solo pregunta por la real) y enviarla seguiría siendo un acto de consentimiento autenticado.
    - **Los ítems antiguos con texto también lo piden** al editarse: seguir tratando ese texto es seguir tratándolo — fricción de una sola vez.
    - El registro **entra en la exportación** (art. 15) y el seed de la demo lo limpia como al resto.
13. ~~**Sin exportación ni borrado de la cuenta desde la app**~~ — **resuelta** en dos partes (se conserva el número). La **exportación**: enlace «Descargar mis datos» en el pie de `/app` que baja un JSON con todo lo del usuario (correo, perfil, categorías, rutina completa con notas, completados, conversación entera y el ajuste de IA **sin la clave**: ni cifrada — un blob inútil fuera del servidor no es un dato del usuario, es una tentación de soporte). Sirve el acceso y la portabilidad (art. 15 y 20) en autoservicio y es la mitigación natural del apartado 9 de los términos, que no garantiza recuperación de datos. Decisiones que importan:
    - `GET /api/export` con la sesión de cookies y la RLS de siempre; fichero `rutia-datos-YYYY-MM-DD.json` (fecha en el huso del perfil), claves en español porque el destinatario es el usuario.
    - **Lecturas paginadas con recuento exacto de control**: PostgREST corta en Max Rows (1000) *sin error*, y un export parcial que dice ser completo desmentiría a la política justo en lo que promete; si el recuento no cuadra, falla ruidosamente (`lib/pagination.ts`).
    - **Respuesta en streaming**: las respuestas no-streaming de Vercel están capadas a 4,5 MB y la conversación se guarda completa e indefinida — el usuario con más datos sería justo aquel al que le fallaría su descarga RGPD.
    - **Fuera del fichero, documentado en la política**: fechas técnicas de registro/inicios de sesión y grants OAuth del MCP (viven en el schema `auth` de Supabase, fuera del alcance de la sesión del propio usuario); se atienden por correo.
    - En la **web app instalada de iOS** no hay gestor de descargas y el `attachment` acaba en una pantalla trampa: el pie lo detecta y da instrucciones (abrir en Safari) en vez de un enlace que parece roto.
    **El borrado total (art. 17) también está resuelto** — «Borrar mi cuenta» en el pie de `/app`, con sus decisiones:
    - **Migración 0005**: función `public.delete_my_account()` `security definer` (propietario `postgres`, que sí puede tocar `auth.users`), ejecutable solo por `authenticated` y que borra únicamente `auth.uid()` — imposible borrar a otro. Además **rechaza los tokens del flujo OAuth** (claim `client_id`, el mismo criterio que 0004): sin esa guarda, un cliente MCP podría llamar a `/rest/v1/rpc/delete_my_account` contra PostgREST y destruir la cuenta saltándose la confirmación, porque las dos barreras solo existen en la capa Next.js. La cascada hace el resto: todas las tablas públicas cuelgan de `auth.users` con `on delete cascade`, y las internas de auth (sesiones, identidades, grants OAuth del MCP) caen con el usuario por el propio esquema de Supabase. **Requiere ejecutar la migración en el SQL Editor**, como 0003 y 0004.
    - **Confirmación de dos barreras en un diálogo propio**: explica qué se pierde y exige teclear `BORRAR` (validado también en el servidor con Zod, como toda frontera). No hay «deshacer»: el texto lo dice.
    - Tras borrar: `signOut()` de cortesía (la sesión ya no tiene dueño) y redirección a la portada.
    - **Ventana honesta de los tokens MCP**: los grants mueren con el usuario y ningún token se puede refrescar, pero un token de acceso ya emitido sigue siendo criptográficamente válido hasta su expiración (≤1 h). Inofensivo en la práctica —la RLS ya no encuentra filas de un usuario que no existe—, y documentado en la política en vez de escondido.
