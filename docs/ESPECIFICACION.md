...
13. **~~Sin exportación ni~~ Sin borrado de la cuenta desde la app** — la **exportación está resuelta**: enlace «Descargar mis datos» en el pie de `/app` que baja un JSON con todo lo del usuario (correo, perfil, categorías, rutina completa con notas, completados, conversación entera y el ajuste de IA **sin la clave**: ni cifrada — un blob inútil fuera del servidor no es un dato del usuario, es una tentación de soporte). Sirve el acceso y la portabilidad (art. 15 y 20) en autoservicio y es la mitigación natural del apartado 9 de los términos, que no garantiza recuperación de datos. Decisiones que importan:
    ...
    **El borrado total (art. 17) sigue por correo**: es una función SQL `security definer` que borre el usuario (todas las tablas cuelgan de `auth.users` con `on delete cascade`) más la revocación de grants MCP, y queda como pieza propia.
