...
16. **La portada sigue siendo la plantilla de `create-next-app`** — `/` no describe RutIA ni enlaza los textos legales, así que quien llegue a la raíz no encuentra la política de privacidad sin pasar por `/login`. No es deuda de arquitectura, es una página sin escribir; se resuelve cuando haya portada real.
