...
- **El mismo diálogo ofrece el modo MCP** (§6.5), y no un botón aparte, porque es la *alternativa* a pegar la clave: quien llega ahí sin clave —el chat le manda— debe ver las dos formas de usar el asistente. Las dos viven en **pestañas al mismo nivel** («Clave de API» | «Conectores», con el alternador `PanelTab` compartido con el panel Chat/Hoy): son caminos alternativos, no una sección principal con un apéndice. El aviso del chat sin clave menciona ambos caminos, cumpliendo lo que §6.4 ya exige (sin clave, el chat ofrece las dos salidas: pegar una clave propia o el modo MCP); la mención al MCP —en el aviso y en las pestañas— solo aparece si el despliegue lo ofrece. Con un guardado en vuelo las pestañas se bloquean, igual que Escape y «Cerrar»: cambiar de vista desmontaría el aviso de error y se perdería sin verse; y cambiar de pestaña desarma la confirmación en dos pasos de «Borrar clave». Lleva un enlace de un clic que abre en Claude el diálogo de conector con la URL ya rellenada, la URL copiable para ChatGPT o un editor, y el aviso de que el acceso concedido llega hasta donde llega la propia sesión (§6.5, §12.15). La URL sale de `MCP_RESOURCE_URL` resuelta **en el servidor** y viaja como prop: no es secreta (se publica sin autenticación en los metadatos RFC 9728), pero la variable no lleva prefijo `NEXT_PUBLIC_`, así que leerla desde el componente de cliente daría `null` y la sección desaparecería sin explicación. Si no está configurada, la sección no se pinta: contarle a un usuario final que existe algo que su despliegue no tiene solo estorba.
